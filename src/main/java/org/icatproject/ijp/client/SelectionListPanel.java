...
public class SelectionListPanel extends Composite {
	...
	private static SelectionListPanelUiBinder uiBinder = GWT.create(SelectionListPanelUiBinder.class);

	interface SelectionListPanelUiBinder extends UiBinder<Widget, SelectionListPanel> {
	}
	...
	// Perhaps this should be called selectionSelectionModel by analogy with
	// selectionListModel :-)
	...
	public SelectionListPanel(String title) {
		...
		// Store the HandlerRegistration of these buttons in case we replace the
		// handler later
		...
		removeSelectedButtonHandler = removeSelectedButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				...
				if (atLeastOneRemoved) {
					...
					// If everything has been removed, disabled the remove
					// buttons
		...
		removeAllButtonHandler = removeAllButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				removeAllButton.setEnabled(false);
				removeSelectedButton.setEnabled(false);
				// Note: will it ever make sense to Accept an empty cart? This
				// assumes that it won't.
				acceptButton.setEnabled(false);
				// Clear will fire a change event if enabled, so no need to fire
				// one here
				clear();
			}
		});
		...
		// Accept button should be enabled iff we have some content; initially
		// we don't
		acceptButton.setEnabled(false);

		// Similarly, the removeSelected and removeAll buttons should be
		// disabled initially
		removeSelectedButton.setEnabled(false);
		removeAllButton.setEnabled(false);

		// Hide and disable the cancel button - caller must use
		// addCancelHandler() to see/enable it
	...
	// Expose the firing mechanism so it can be used in replacement button
	// handlers
	//
	...
	/**
	 * Add the supplied contentItem to the selection table, if it is not already
	 * in there.
	 * 
	 * @param contentItem
	...
	/**
	 * Add the supplied list to the selection list. Duplicates will be ignored.
	 * 
	 * @param contents
	 *            list to add
	 */
	...
	/**
	 * Use the supplied list of contents as the selection list contents. This
	 * will replace any existing contents. Duplicates in the contents will only
	 * be added once.
	 * 
	 * @param contents
	 *            the list
	...
	/**
	 * Change the name and handler for the Remove Selected button. This removes
	 * any previous click handler. Passing a null for ClickHandler will remove
	 * the button.
	 * 
	 * @param name
	 *            the name
	...
	/**
	 * Change the name and handler for the Remove All button. This removes any
	 * previous click handler. Passing a null for ClickHandler will remove the
	 * button.
	 * 
	 * @param name
	 *            the name
	...
	public void setVisible(boolean b) {
		...
		// The Remove Selected / All buttons should only be shown if they have
		// defined handlers
